...
def main(prefix: str, chunk_size: int = 300, max_text_len: int = 800):
    ...
    for idx, segment in enumerate(split_segments):
        ...
        if len(logits) > max_tokens:
            max_tokens = len(logits)
        print(
            f"Segment {idx}: {len(logits)} tokens, {len(segment['text'])} characters, {int(segment['end']-segment['start'])} seconds"
        )
